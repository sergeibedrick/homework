...
# Inline-клавиатура для покупки продуктов
...
async def get_buying_list(message: types.Message):
    for i in range(1, 5):
        ...
        try:
            photo = FSInputFile(f"product{i}.png") # картинки в той же
            # директории, что и скрипт
            await message.answer_photo(photo=photo, caption=text)
        except FileNotFoundError: #  если картинки не найдены
            await message.answer(text=text)

    await message.answer("Выберите продукт для покупки:", reply_markup=buying_keyboard)


# Обработчик callback для покупки продукта
